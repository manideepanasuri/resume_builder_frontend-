...
import PositionsComponent from "./PostionsComoponet";
import { PositionForm } from "./forms/PositionsForm";
import { useState } from "react";

export default function Positions() {
  const { positions } = AllDetailsStore();
      ...
      <ScrollArea>
        <div className="flex w-10 items-stretch gap-2 ">
          {!positions ? (
            ...
            positions.map((edu) => (
              <PositionsComponent position={edu} key={edu.id} />
            ))
          )}
          <div
          ...
          >
            ...
            <PositionForm open={open} setOpen={setOpen} position={null} />
          </div>
        </div>
